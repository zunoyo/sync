...
export default function Playlists() {
  ...
  const featuredPlaylists = visiblePlaylists.slice(0, 2);
  ...
  return (
    <>
      ...
      {/* Featured 히어로 배너 (첫 번째 플레이리스트) */}
      {!loading && showPl && visiblePlaylists.length > 0 && (
        <Link
          to={`/playlists/${visiblePlaylists[0].id}`}
          className="pl-hero-banner"
          style={{ textDecoration: 'none', color: 'inherit' }}
        >
          <div className="pl-hero-art">
            {visiblePlaylists[0].coverImage
              ? <img src={visiblePlaylists[0].coverImage} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 12 }} />
              : <PlaylistCoverMosaic tracks={visiblePlaylists[0].tracks || []} size="100%" />
            }
          </div>
          <div className="pl-hero-info">
            <div className="pl-hero-tag">{visiblePlaylists[0].public ? '공유중' : '내 플레이리스트'}</div>
            <div className="pl-hero-name">{visiblePlaylists[0].playlistName}</div>
            <div className="pl-hero-meta">{counts[visiblePlaylists[0].id] !== undefined ? `${counts[visiblePlaylists[0].id]}곡` : ''}</div>
          </div>
        </Link>
      )}
      ...
      {!loading && showPl && (
        <div className="pl-featured">
          {featuredPlaylists.map((pl) => (
            <Link key={pl.id} to={`/playlists/${pl.id}`} className={`pl-feat-card ${plGradient(pl)}`} style={{ textDecoration: 'none' }}>
              <span className="pl-feat-tag">{pl.public ? '공유중' : '내 플레이리스트'}</span>
              <div className="pl-feat-name">{pl.playlistName}</div>
              <div className="pl-feat-meta">{counts[pl.id] !== undefined ? `${counts[pl.id]}곡` : ''}</div>
              <button className="pl-feat-play" onClick={(e) => e.preventDefault()}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>
              </button>
            </Link>
          ))}
          <div className="pl-create-tile" onClick={() => setModalOpen(true)}>
      ...
      <div className="cards-grid" id="playlist-grid">
        ...
        {!loading && showPl && <div className="pl-section-label" style={{ gridColumn: '1/-1' }}>내 플레이리스트</div>}
        ...
        {!loading && showAr && filter === 'all' && <div className="pl-section-label" style={{ gridColumn: '1/-1' }}>아티스트</div>}
        ...
        {!loading && showAl && filter === 'all' && <div className="pl-section-label" style={{ gridColumn: '1/-1' }}>앨범</div>}
...
function PlaylistCard({ pl, count, onDelete }) {
  ...
  return (
    <Link to={`/playlists/${pl.id}`} className="playlist-card" style={{ textDecoration: 'none', color: 'inherit' }}>
      <div className={`playlist-card-art ${pl.coverImage ? '' : plGradient(pl)}`}>
        {pl.coverImage
          ? <img src={pl.coverImage} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          : (pl.tracks && pl.tracks.length > 0
              ? <PlaylistCoverMosaic tracks={pl.tracks} size="100%" />

...
export default function AlbumDetail() {
  ...
  return (
    <>
      <div className="detail-hero h-album">
        <div className="detail-hero-bg" style={{ backgroundImage: album?.albumArt ? `url(${album.albumArt})` : undefined }} />
        <div className="detail-hero-content">
          <div className="detail-hero-art">
            {album?.albumArt
              ? <img src={album.albumArt} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              : '💿'}
          </div>
          <div className="detail-hero-info">
            <div className="detail-hero-type">앨범</div>
            <h1 className="detail-hero-name">{album?.albumName || '불러오는 중...'}</h1>
            <div className="detail-hero-meta">
              {album?.artistName && <span>{album.artistName}</span>}
              {album?.artistName && <span>·</span>}
              <span>{album ? `${album.tracks.length} TRACKS${album.releaseYear ? ` · ${album.releaseYear}` : ''}` : '불러오는 중...'}</span>
            </div>
            <div className="detail-action-row">
              ...
              <button
                ...
                disabled={busy}
                onClick={toggleSave}
                style={{ borderColor: savedEntry ? 'var(--accent)' : undefined, color: savedEntry ? 'var(--accent)' : undefined }}
              >

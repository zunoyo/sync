    ...
    <>
      ...
      {requestCount > 0 && (
        <div className="req-banner">
          <div className="req-banner-icon">🔔</div>
          <div className="req-banner-txt">
            <b>{requestCount}명이 친구 요청을 보냈어요</b>
            <span>수락하면 서로의 플레이리스트를 볼 수 있어요</span>
          </div>
          <button className="pill-btn accent" onClick={() => setShowRequests(true)}>확인하기</button>
        </div>
      )}
          ...
          {friends && friends.map((f) => {
            ...
            return (
              <div className="friend-card" key={f.id} onClick={() => setSelectedFriend({ id: u.id, name })}>
                <div className={`friend-avatar ${friendGradient(u.id)}`}>
                  {friendInitials(name)}
                </div>

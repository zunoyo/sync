...
/** 관련 아티스트 — Last.fm 유사 아티스트 + iTunes 이미지 보강 (우측 패널용) */
export async function fetchRelatedArtists(artistName) {
  try {
    const res = await fetch(
      `https://ws.audioscrobbler.com/2.0/?method=artist.getsimilar&artist=${encodeURIComponent(artistName)}&api_key=${LASTFM_API_KEY}&format=json&limit=5`,
      { cache: 'no-store' }
    );
    ...
    // Last.fm 이미지는 대부분 깨져있어서 iTunes로 대체 보강 — 실패 시 재시도
    return await Promise.all(list.map(async (a) => {
      const d2 = await fetchItunesJson(`/api/itunes-proxy/search?term=${encodeURIComponent(a.name)}&media=music&entity=song&limit=1`);
      const image = hiResArt(d2?.results?.[0]?.artworkUrl100, 200) || null;
...
/** iTunes raw 검색 결과(같은 아티스트로 필터링된 상태) → 앨범별 그룹화 + flatTracks 구성 (공용) */
function groupItunesResultsIntoAlbums(results, canonicalArtistId, bio) {
  const firstArt = results.find((r) => r.artworkUrl100)?.artworkUrl100;
  const image = firstArt ? hiResArt(firstArt, 400) : null;
...
async function fetchLastfmBio(artistName) {
  try {
    const data = await fetch(`https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=${encodeURIComponent(artistName)}&api_key=${LASTFM_API_KEY}&format=json&lang=ko`)
      .then((r) => r.json());
    if (!data?.artist?.bio?.summary) return null;
    const cleaned = data.artist.bio.summary
      .replace(/<a[^>]*>.*?<\/a>/gi, '').replace(/<[^>]+>/g, '')
      .replace(/Read more on Last\.fm\.?/gi, '').trim().slice(0, 120);
    return cleaned ? cleaned + (cleaned.length >= 120 ? '...' : '') : null;
  } catch {
    return null;
  }
}

/** 저장된 아티스트를 다시 열 때 쓰는 경로 — 이름 재검색(퍼지 매칭) 대신 저장 당시 확보해둔
 *  iTunes artistId로 직접 lookup한다. 정확하고, 한글 표기명이 iTunes 검색어와 잘 안 맞는
 *  아티스트(K-pop 그룹 등)에서도 흔들리지 않는다. lookup이 비거나 실패하면(오래돼서 ID가
 *  무효해진 경우 등) 기존 이름 검색 경로로 자동 폴백한다. */
export async function fetchArtistDetailById(externalId, artistName) {
  if (!externalId) return fetchArtistDetail(artistName);

  const [lookupData, bio] = await Promise.all([
    fetchItunesJson(`/api/itunes-proxy/lookup?id=${encodeURIComponent(externalId)}&entity=song&limit=200`),
    fetchLastfmBio(artistName),
  ]);

  const results = (lookupData.results || []).filter((r) => r.wrapperType === 'track' || r.trackName);
  if (results.length === 0) return fetchArtistDetail(artistName);

  return groupItunesResultsIntoAlbums(results, externalId, bio);
}

/** 아티스트 상세 데이터: iTunes 곡 전체 + Last.fm 바이오, 앨범별로 그룹화해 반환 */
export async function fetchArtistDetail(artistName) {
  const [itunesData, bio] = await Promise.all([
    fetchItunesJson(`/api/itunes-proxy/search?term=${encodeURIComponent(artistName)}&media=music&entity=song&limit=100`),
    fetchLastfmBio(artistName),
  ]);
  ...
  return groupItunesResultsIntoAlbums(results, canonicalArtistId, bio);
}
...
export async function fetchAlbumDetail(externalId, hintArtistName, hintAlbumName) {
  const data = await fetchItunesJson(`/api/itunes-proxy/lookup?id=${encodeURIComponent(externalId)}&entity=song`);
  ...
  const tracks = results
    ...
    .map((r, i) => ({
      ...
      albumArt: hiResArt(r.artworkUrl100 || collection?.artworkUrl100, 500) || null,
      durationMs: r.trackTimeMillis || 0,
      duration: r.trackTimeMillis
        ? `${Math.floor(r.trackTimeMillis / 60000)}:${String(Math.floor((r.trackTimeMillis % 60000) / 1000)).padStart(2, '0')}`
        : '—',
  ...
  return {
    ...
    albumArt: hiResArt(collection?.artworkUrl100, 600) || tracks[0]?.albumArt || null,

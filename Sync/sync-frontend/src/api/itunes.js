...
/** iTunes/Apple 아트 URL 해상도 업그레이드 — url.replace('100x100bb', '600x600bb') 식의
 *  리터럴 문자열 치환은 iTunes가 100x100이 아닌 다른 해상도로 내려줄 때(예: 60x60bb) 조용히
 *  실패해서 저화질 그대로 표시되는 문제가 있었다. 정규식으로 "숫자x숫자bb" 패턴 자체를
 *  찾아 치환하면 원본 해상도가 무엇이든 항상 동작한다. */
...
export async function fetchItunesTrack(query) {
  ...
  try {
    const res = await fetch(`/api/itunes-proxy/search?term=${encodeURIComponent(query)}&media=music&entity=song&limit=1`, { signal: controller.signal, cache: 'no-store' });
...
export async function searchItunesTracks(term, limit = 20) {
  try {
    const res = await fetch(`/api/itunes-proxy/search?term=${encodeURIComponent(term)}&media=music&entity=song&limit=${limit}`, { cache: 'no-store' });
    const data = await res.json();
    return data.results || [];
  } catch {
    return [];
...
export async function searchItunesArtists(term, limit = 6) {
  try {
    const res = await fetch(`/api/itunes-proxy/search?term=${encodeURIComponent(term)}&entity=musicArtist&limit=${limit}`, { cache: 'no-store' });
    const data = await res.json();

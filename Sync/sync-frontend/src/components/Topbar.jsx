...
export default function Topbar() {
  ...
  const displayName = user?.displayName || user?.username || 'SYNC 사용자';
    ...
    <header className="topbar">
      ...
      <div className="topbar-right">
        <div style={{ position: 'relative' }}>
          ...
          {isHomePage && (
            <div className="home-mode-toggle" id="home-mode-toggle">
              <button className={galaxyOpen ? '' : 'active'} onClick={() => setGalaxyOpen(false)}>⊞ 그리드 뷰</button>
              <button className={galaxyOpen ? 'active' : ''} onClick={() => setGalaxyOpen(true)}>🌌 갤럭시 뷰</button>
            </div>
          )}
        </div>

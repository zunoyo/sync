...
export function ConfidenceGauge({ pct, color = 'var(--accent)' }) {
  const r = 30, cx = 36, cy = 36;
  const circ = 2 * Math.PI * r;
  const offset = circ * (1 - Math.max(0, Math.min(100, pct)) / 100);
  return (
    <svg viewBox="0 0 72 72" style={{ width: 64, height: 64, flexShrink: 0 }}>
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="rgba(245,238,240,.1)" strokeWidth="7" />
      <circle
        cx={cx} cy={cy} r={r} fill="none" style={{ stroke: color }}
...
export function VAPlot({ valence, arousal }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxWidth: 230, height: 'auto', display: 'block', margin: '0 auto' }}>
      <rect x={PAD} y={PAD} width={pw} height={ph} fill="rgba(245,238,240,.03)" stroke="rgba(245,238,240,.12)" strokeWidth="1" rx="6" />
      <line x1={PAD} y1={PAD + ph / 2} x2={PAD + pw} y2={PAD + ph / 2} stroke="rgba(245,238,240,.15)" strokeWidth="1" />
      <line x1={PAD + pw / 2} y1={PAD} x2={PAD + pw / 2} y2={PAD + ph} stroke="rgba(245,238,240,.15)" strokeWidth="1" />
      <text x={PAD + 8} y={PAD + 16} fontSize="9" fill="rgba(245,238,240,.38)">화남·불안</text>
      <text x={PAD + pw - 8} y={PAD + 16} fontSize="9" fill="rgba(245,238,240,.38)" textAnchor="end">신남·기쁨</text>
      <text x={PAD + 8} y={PAD + ph - 8} fontSize="9" fill="rgba(245,238,240,.38)">우울·슬픔</text>
      <text x={PAD + pw - 8} y={PAD + ph - 8} fontSize="9" fill="rgba(245,238,240,.38)" textAnchor="end">평온·만족</text>
      <text x={PAD - 6} y={PAD + ph / 2 + 3} fontSize="10" fill="rgba(245,238,240,.55)" textAnchor="end">부정</text>
      <text x={PAD + pw + 6} y={PAD + ph / 2 + 3} fontSize="10" fill="rgba(245,238,240,.55)">긍정</text>
      <text x={PAD + pw / 2} y={PAD - 10} fontSize="10" fill="rgba(245,238,240,.55)" textAnchor="middle">활발</text>
      <text x={PAD + pw / 2} y={PAD + ph + 20} fontSize="10" fill="rgba(245,238,240,.55)" textAnchor="middle">차분</text>
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
      <tbody>
        {rows.map(([label, value]) => (
          <tr key={label} style={{ borderBottom: '1px solid rgba(245,238,240,.06)' }}>
...
export default function AnalysisDetail({ emotion }) {
  ...
  const acc = emotion.modelValAccuracy != null ? Math.round(emotion.modelValAccuracy * 100) : null;
  const hasVA = emotion.valence != null && emotion.arousal != null;

  return (
    <div style={{ marginTop: 16, padding: 16, background: 'rgba(245,238,240,.04)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(245,238,240,.07)' }}>
      ...
      <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ flexShrink: 0, display: 'flex', gap: 14 }}>
          ...
          {acc != null && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              <ConfidenceGauge pct={acc} color="var(--info)" />
              <span style={{ fontSize: 10, color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>모델 정확도</span>
            </div>
          )}
        </div>
      </div>
      ...
      {tags.length > 0 && (
        <>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
            {tags.map((tag) => (
              <span key={tag} style={{ background: 'rgba(245,238,240,.08)', padding: '3px 10px', borderRadius: 'var(--radius-full)', fontSize: 12, color: '#fff' }}>#{tag}</span>
            ))}
          </div>
        </>
      )}

...
export default function AlbumDetail() {
  ...
  // 백드롭 사진 프리로드 — 앨범아트가 생기는 즉시 opacity:1로 렌더하면 이미지가 실제로
  // 로드되는 순간 뚝 끊기듯 나타나서 느리게 느껴짐. 디코딩이 끝난 뒤에만 페이드인시킨다.
  useEffect(() => {
    setBgLoaded(false);
    if (!album?.albumArt) return;
    let cancelled = false;
    const img = new Image();
  ...
  return (
    <>
      <div className="detail-hero grad-4" style={{ position: 'relative', overflow: 'hidden' }}>
        {album?.albumArt && (
          <div style={{
            position: 'absolute', inset: 0,
            backgroundImage: `url(${album.albumArt})`, backgroundSize: 'cover', backgroundPosition: 'center',
            filter: 'blur(60px) brightness(.4) saturate(1.5)', transform: 'scale(1.1)',
            opacity: bgLoaded ? 1 : 0, transition: 'opacity .5s ease', zIndex: 0,
          }} />
        )}
        <div className="detail-hero-content" style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'flex-end', gap: 40, width: '100%' }}>

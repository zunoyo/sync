...
import { fetchPlaylists, fetchPlaylistTracks, deletePlaylist, plGradient, plEmoji, serverTrackToApp } from '../api/playlists';
  ...
  async function load() {
    ...
    if (pls.length) {
      const entries = await Promise.all(pls.map(async (pl) => {
        try {
          const raw = await fetchPlaylistTracks(pl.id);
          return [pl.id, raw.map(serverTrackToApp)];
        } catch {
          return [pl.id, []];
        }
      }));
      setCounts((prev) => ({ ...prev, ...Object.fromEntries(entries.map(([id, tr]) => [id, tr.length])) }));
      setPlTracks((prev) => ({ ...prev, ...Object.fromEntries(entries) }));
    }
    setLoading(false);
  }
  ...
  return (
    <>
      <div className="section-header" style={{ marginBottom: 24 }}>
        <h2 className="pl-page-title">플레이리스트 관리</h2>
      ...
      <div id="playlist-hero-wrap">
        {!loading && showPl && (
          <PlaylistHero playlists={visiblePlaylists} counts={counts} plTracks={plTracks} onCreate={() => setModalOpen(true)} />
        )}
      </div>
...
/** 대표 플레이리스트 히어로 배너 + Featured 카드(상위 2개) + 생성 타일 */
function PlaylistHero({ playlists, counts, plTracks, onCreate }) {
  const first = playlists[0];
  const featured = playlists.slice(0, 2);

  return (
    <>
      {first && (
        <Link to={`/playlists/${first.id}`} className="pl-hero-banner" style={{ textDecoration: 'none', color: 'inherit' }}>
          <div className="pl-hero-art">
            {first.coverImage ? (
              <img src={first.coverImage} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { e.target.style.display = 'none'; }} />
            ) : plTracks[first.id]?.some((t) => t.albumArt) ? (
              <PlaylistCoverMosaic tracks={plTracks[first.id]} />
            ) : (
              <span style={{ fontSize: 40 }}>{plEmoji(first)}</span>
            )}
          </div>
          <div className="pl-hero-info">
            <div className="pl-hero-tag">{first.public ? '공유중' : '내 플레이리스트'}</div>
            <div className="pl-hero-name">{first.playlistName}</div>
            <div className="pl-hero-meta">{counts[first.id] ?? 0}곡</div>
          </div>
        </Link>
      )}
      ...
      <div className="pl-featured">
        {featured.map((pl) => (
          <Link key={pl.id} to={`/playlists/${pl.id}`} className={`pl-feat-card ${plGradient(pl)}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            <span className="pl-feat-tag">{pl.public ? '공유중' : '내 플레이리스트'}</span>
            <div className="pl-feat-name">{pl.playlistName}</div>
            <div className="pl-feat-meta">{counts[pl.id] ?? 0}곡</div>
            <span className="pl-feat-play">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>
            </span>
          </Link>
        ))}
        <div className="pl-create-tile" onClick={onCreate}>
          <div className="plus">＋</div>
          <div className="lbl">새 플레이리스트 만들기</div>
          <div className="sub">좋아하는 곡들을 모아보세요</div>
        </div>
      </div>
    </>
  );
    ...
    <Link to={`/playlists/${pl.id}`} className="playlist-card" style={{ textDecoration: 'none', color: 'inherit' }}>
      <div className={`playlist-card-art ${plGradient(pl)}`}>
        ...
        {shared && (
          <span
            title="친구에게 공유중"
            style={{ position: 'absolute', top: 8, left: 8, background: 'rgba(217,192,143,.9)', color: '#000', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 'var(--radius-full)' }}
          >
            공유중
          </span>
        )}
...
function ArtistCard({ a, onDelete }) {
  return (
    <Link to={`/artist/${encodeURIComponent(a.artistName)}`} state={{ externalId: a.artistExternalId }} className="playlist-card" style={{ '--card-radius': '50%', textDecoration: 'none', color: 'inherit' }}>

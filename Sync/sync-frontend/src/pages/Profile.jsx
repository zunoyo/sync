    ...
    <>
      <div className="profile-hero">
        ...
        <div className="profile-info">
          ...
          <div className="profile-name">{user?.displayName || user?.username || 'SYNC 사용자'}</div>
      ...
      {/* 기본 정보 */}
      <div className="settings-section span2">
      ...
      <div className="settings-section">
        ...
        <ToggleRow label="이메일 뉴스레터" desc="SYNC 소식 및 추천" checked={notif.email} onChange={(v) => setNotif((n) => ({ ...n, email: v }))} />
...
function SpotifyStatusSection({ status, onChanged }) {
  ...
  if (status.connected) {
    ...
    return (
      <div style={{
        ...
        background: expired ? 'rgba(255,165,0,.08)' : 'rgba(217,192,143,.08)',
        border: `1px solid ${expired ? 'rgba(255,165,0,.3)' : 'rgba(217,192,143,.2)'}`, borderRadius: 12,
      }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: expired ? '#ffa500' : 'var(--accent)' }}>
            {expired ? '⚠ 토큰 만료' : '✅ 연동됨'}
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {expired && (
            <button onClick={reconnect} style={{ background: 'var(--accent)', border: 'none', color: '#000', padding: '8px 16px', borderRadius: 50, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
              재연동하기
            </button>
          )}
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 16, background: 'var(--bg-mid)', borderRadius: 12, border: '1px solid rgba(255,255,255,.07)' }}>
      ...
      <button onClick={connect} style={{ background: 'var(--accent)', border: 'none', color: '#000', padding: '8px 18px', borderRadius: 50, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
        Spotify 연동
      </button>
    </div>
  );

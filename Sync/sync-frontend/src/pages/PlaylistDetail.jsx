...
import {
  fetchPlaylist, fetchPlaylistTracks, serverTrackToApp, plEmoji,
...
export default function PlaylistDetail() {
  ...
  return (
    <>
      <div className={`detail-hero h-playlist`}>
        {playlist?.coverImage && <div className="detail-hero-bg" style={{ backgroundImage: `url(${playlist.coverImage})` }} />}
        <div className="detail-hero-content">
          <div className="detail-hero-art">
            {playlist?.coverImage
              ? <img src={playlist.coverImage} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { e.target.style.display = 'none'; }} />
              : (playlist ? plEmoji(playlist) : '🎵')}
          </div>
          <div className="detail-hero-info">
            <div className="detail-hero-type">플레이리스트</div>
            <h1 className="detail-hero-name">{playlist ? playlist.playlistName : '불러오는 중...'}</h1>
            <div className="detail-hero-meta">{tracks ? <span>{tracks.length} TRACKS</span> : ''}</div>
            <div className="detail-action-row">
              ...
              <button
                ...
                style={{ color: shared ? 'var(--accent)' : '', borderColor: shared ? 'var(--accent)' : '' }}
              >

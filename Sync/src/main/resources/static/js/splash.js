...
const Splash = (function () {
  ...
  /* 랜딩 화면 뒤에 숨어있던 스플래시를 이제 막 드러내는 시점(랜딩이 막 페이드를 시작할 때) —
     display를 되돌려 아이콘 바운스/타이틀 페이드 등 애니메이션이 처음부터 재생되게 한다.
     이 시점에는 아직 2300ms 카운트다운은 시작하지 않는다(reveal과 start를 분리) — 랜딩이
     완전히 사라지기도 전에 스플래시가 먼저 사라지기 시작하면 안 되니까. */
  function reveal() {
    const splash = document.getElementById('splash-screen');
...
(function () {
  ...
  function boot() {
    if (typeof LandingView !== 'undefined' && LandingView.shouldShow()) {
      /* 랜딩이 먼저 뜨는 동안 스플래시(및 그 애니메이션)는 완전히 숨겨둔다 */
      if (splash) splash.style.display = 'none';
      LandingView.show(Splash.reveal, Splash.start);
    } else {
      Splash.start();
    }
  }

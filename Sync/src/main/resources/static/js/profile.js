...
const ProfileModule = (() => {
  ...
  function _toast(msg, type = 'success') {
    const colors = {
      success: { bg:'rgba(217,192,143,.15)', border:'rgba(217,192,143,.4)', icon:'✅' },
      error:   { bg:'rgba(224,99,122,.15)', border:'rgba(224,99,122,.4)', icon:'❌' },
      info:    { bg:'rgba(179,82,122,.15)',  border:'rgba(179,82,122,.4)',  icon:'ℹ️' },
    };
  ...
  function validatePw() {
    ...
    if (hint) {
      hint.style.color = newPw.length === 0 ? 'var(--text-muted)'
                       : strong ? 'var(--accent)' : 'var(--negative)';
    ...
    if (input2 && newPw2.length > 0) {
      input2.style.borderColor = (newPw === newPw2) ? 'var(--accent)' : 'var(--negative)';
    }
  }

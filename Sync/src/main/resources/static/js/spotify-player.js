/**
 * SYNC — Spotify Web Playback SDK Module (v3.1)
...
const SpotifyPlayer = (() => {
    ...
    function _setupPlayer() {
      window.onSpotifyWebPlaybackSDKReady = async () => {
        ...
        _player.addListener('player_state_changed', state => {
          if (!state) return;
          if (!state.paused) {
            if (state.position > 1000) _hasProgressed = true; // 실제로 재생이 진행 중임을 확인
            _startPolling();
          } else {
            _stopPolling();
            ...
            if (_hasProgressed && state.position === 0) {
              // 재생되다가 갑자기 처음(0ms)으로 돌아오며 정지 → 곡이 끝까지 재생되어 자연 종료된 것
              _hasProgressed = false;
              if (typeof Player !== 'undefined' && Player._onSpotifyTrackEnd) Player._onSpotifyTrackEnd();
            }
          }
        });
  ...
  async function play(trackId) {
    ...
    _hasProgressed = false; // 새 트랙 시작 — 이전 트랙의 종료감지 플래그 초기화
  ...
  function _showPremiumNotice() {
    ...
    d.style.cssText = 'position:fixed;bottom:90px;left:50%;transform:translateX(-50%);' +
      'background:rgba(217,192,143,.15);border:1px solid rgba(217,192,143,.4);' +

...
const SavedLibrary = (() => {
  ...
  async function showAlbumDetail(externalId, albumName, artistName, albumArt) {
    ...
    const saved = isAlbumSaved(externalId);
    hero.className = 'detail-hero h-album';
    hero.style.cssText = '';
    hero.innerHTML = `
      <div class="detail-hero-bg" id="album-hero-bg"></div>
      <div class="artist-hero-bg-photo" id="album-hero-bg-photo"></div>
      <div class="detail-hero-content">
        <div class="detail-hero-art">
          ${albumArt ? `<img src="${albumArt}" style="width:100%;height:100%;object-fit:cover">` : '💿'}
        </div>
        <div class="detail-hero-info">
          <div class="detail-hero-type">앨범</div>
          <h1 class="detail-hero-name">${albumName}</h1>
          <div class="detail-hero-meta">
            ${artistName ? `<span>${artistName}</span><span>·</span>` : ''}
            <span id="album-meta-txt">불러오는 중...</span>
          </div>
          <div class="detail-action-row">
            <button class="play-btn-large" id="album-play-btn" disabled title="재생">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
            </button>
            <button class="action-btn" id="album-save-btn" title="앨범 저장"
                    style="${saved ? 'color:var(--accent);border-color:var(--accent)' : ''}"
                    onclick="SavedLibrary._toggleAlbumBtn('${externalId}','${albumName.replace(/'/g,"\\'")}','${(artistName||'').replace(/'/g,"\\'")}','${albumArt||''}')">
    ...
    // 배경(흐림 처리)은 프리로드 후 opacity 트랜지션으로 페이드인 — 아티스트/장르 상세와 동일한 방식.
    // 바로 background-image를 지정하면 큰 이미지가 늦게 뜰 때 화면에 아예 안 뜨는 것처럼 보임
    if (albumArt) {
      const bgPhotoEl = document.getElementById('album-hero-bg-photo');
      if (bgPhotoEl) {
        const preload = new Image();
        preload.onload = () => {
          bgPhotoEl.style.backgroundImage = `url(${albumArt})`;
          bgPhotoEl.classList.add('is-visible');
        };
        preload.src = albumArt;
      }
    }
    ...
    try {
      const data = await _fetchItunesJson(`/api/itunes-proxy/lookup?id=${encodeURIComponent(externalId)}&entity=song`);
      ...
      if (metaEl) metaEl.textContent = `${tracks.length} TRACKS`;
  ...
  /* ── 앨범 상세 히어로의 저장 버튼 클릭 핸들러 ── */
  async function _toggleAlbumBtn(externalId, albumName, artistName, albumArt) {
    const btn = document.getElementById('album-save-btn');
    if (btn) btn.disabled = true;
    try {
      const nowSaved = await toggleAlbum({ externalId, name: albumName, artistName: artistName || null, artUrl: albumArt || null });
      if (btn) btn.style.cssText = nowSaved ? 'color:var(--accent);border-color:var(--accent)' : '';
      if (typeof LibraryFilter !== 'undefined') LibraryFilter.refresh();
    } catch (e) {}
    if (btn) btn.disabled = false;
  }

  return { loadArtists, loadAlbums, isArtistSaved, isAlbumSaved, toggleArtist, toggleAlbum, showAlbumDetail, _toggleAlbumBtn };
})();

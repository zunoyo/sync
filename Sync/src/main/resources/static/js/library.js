...
const DetailPage = (() => {
  ...
  // knownExternalId가 있으면(저장된 아티스트를 다시 열 때) 이름 재검색 대신 그 ID로 바로 조회 — 더 정확함
  function showArtist(name, knownExternalId) {
    if (typeof HomePage !== 'undefined') HomePage._showArtistDetail(name, knownExternalId);
  }
  ...
  async function _populateRealPlaylist(id) {
    ...
    hero.className = 'detail-hero h-playlist';
    hero.style.cssText = '';
    hero.innerHTML = `
      <div class="detail-hero-bg" id="dp-pl-bg"></div>
      <div class="detail-hero-content">
        <div class="detail-hero-art" id="dp-pl-art">🎵</div>
        <div class="detail-hero-info">
          <div class="detail-hero-type">플레이리스트</div>
          <h1 class="detail-hero-name" id="dp-pl-name">불러오는 중...</h1>
          ...
          <div class="detail-action-row">
            ...
            <button class="action-btn" id="dp-pl-share" title="친구 공유 켜기" onclick="DetailPage._toggleVisibility(${id})">
    ...
    try {
      ...
      if (metaEl) metaEl.innerHTML = `<span>${tracks.length} TRACKS</span>`;
      const artEl = document.getElementById('dp-pl-art');
      if (artEl) artEl.textContent = _plEmoji(pl);
      const bgEl = document.getElementById('dp-pl-bg');
      if (bgEl && pl.coverUrl) bgEl.style.backgroundImage = `url(${pl.coverUrl})`;
  ...
  function _setShareButton(btn, isPublic) {
    ...
    btn.style.color       = isPublic ? 'var(--accent)' : '';
    btn.style.borderColor = isPublic ? 'var(--accent)' : '';
  }

  async function _toggleVisibility(id) {
    const btn = document.getElementById('dp-pl-share');
    const wantPublic = btn?.dataset.public !== '1';
    ...
    const groups = [
      ...
      { kind: 'artist', label: '아티스트', html: artists.map(a => `
        <div class="library-item" data-kind="artist"
             onclick="DetailPage.showArtist('${String(a.artistName).replace(/'/g, "\\'")}', ${a.artistExternalId ? `'${String(a.artistExternalId).replace(/'/g, "\\'")}'` : 'null'})">
...
const PlaylistPage = (() => {
  ...
  async function render(filter) {
    ...
    const heroWrap = document.getElementById('playlist-hero-wrap');

    let html = '';

    if (showPl) {
      if (heroWrap) heroWrap.innerHTML = _heroAndFeatured(playlists);
      html += _sectionLabel('내 플레이리스트');
  ...
  /* ── 히어로 배너(대표 플레이리스트 1개) + Featured 카드(상위 2개) + 생성 타일 ── */
  function _heroAndFeatured(playlists) {
    let out = '';
    const first = playlists[0];

    if (first) {
      const emoji = first.emoji || _hashPick('e' + first.id, EMOJIS);
      const count = _plCounts[first.id] ?? 0;
      const tag   = first.public ? '공유중' : '내 플레이리스트';
      out += `
        <div class="pl-hero-banner" onclick="DetailPage.showPlaylist(${first.id})">
          <div class="pl-hero-art">
            ${first.coverImage
              ? `<img src="${first.coverImage}" style="width:100%;height:100%;object-fit:cover" onerror="this.style.display='none'">`
              : `<span style="font-size:40px">${emoji}</span>`}
          </div>
          <div class="pl-hero-info">
            <div class="pl-hero-tag">${tag}</div>
            <div class="pl-hero-name">${first.playlistName}</div>
            <div class="pl-hero-meta">${count}곡</div>
          </div>
        </div>`;
    }

    out += `<div class="pl-featured">`;
    out += playlists.slice(0, 2).map(pl => {
      const grad  = pl.gradient || _hashPick('g' + pl.id, GRADS);
      const count = _plCounts[pl.id] ?? 0;
      const tag   = pl.public ? '공유중' : '내 플레이리스트';
      return `
        <div class="pl-feat-card ${grad}" onclick="DetailPage.showPlaylist(${pl.id})">
          <span class="pl-feat-tag">${tag}</span>
          <div class="pl-feat-name">${pl.playlistName}</div>
          <div class="pl-feat-meta">${count}곡</div>
          <button class="pl-feat-play" onclick="event.stopPropagation();DetailPage.showPlaylist(${pl.id})">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
          </button>
        </div>`;
    }).join('');
    out += `
        <div class="pl-create-tile" onclick="CreatePlaylistModal.show()">
          <div class="plus">＋</div>
          <div class="lbl">새 플레이리스트 만들기</div>
          <div class="sub">좋아하는 곡들을 모아보세요</div>
        </div>`;
    out += `</div>`;
    return out;
  }
      ...
      <div class="playlist-card" onclick="DetailPage.showPlaylist(${pl.id})">
        <div class="playlist-card-art ${grad}">
          ...
          ${shared ? `<span title="친구에게 공유중" style="position:absolute;top:8px;left:8px;
                       background:rgba(217,192,143,.9);color:#000;font-size:10px;font-weight:700;
  ...
  function _artistCard(a) {
    ...
    const extIdArg = a.artistExternalId ? `'${String(a.artistExternalId).replace(/'/g, "\\'")}'` : 'null';
    return `
      <div class="playlist-card" onclick="DetailPage.showArtist('${nameEsc}', ${extIdArg})" style="--card-radius:50%">
        <div class="playlist-card-art ${_hashPick('a' + a.artistName, GRADS)}" style="border-radius:50%;overflow:hidden">
          ...
          <div class="playlist-card-overlay" style="border-radius:50%">
            <button class="detail-play-btn" title="보기" onclick="event.stopPropagation();DetailPage.showArtist('${nameEsc}', ${extIdArg})">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
            </button>
          </div>
        </div>
  ...
  function _sectionLabel(label) {
    return `<div class="pl-section-label" style="grid-column:1/-1">${label}</div>`;
  }

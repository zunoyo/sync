...
const AuthModal = (() => {
    ...
    prompt.innerHTML = `
      <div style="text-align:center;padding:16px 0 20px">
        ...
        <button onclick="try{localStorage.setItem('sw_spotify_redirect','1')}catch(e){}; window.location.href='/api/spotify/connect'"
                style="width:100%;background:var(--accent);border:none;color:#000;
  ...
  function onPwInput(input) {
    ...
    const colors = ['', 'var(--negative)', 'var(--warning)', 'var(--warning)', 'var(--accent)', 'var(--accent)'];
  ...
  function _refreshTopbar() {
    ...
    const displayName = user.display_name || user.username || 'SYNC 사용자';
  ...
  function _showToast(msg, type) {
    const colors  = { success:'rgba(217,192,143,.15)', error:'rgba(224,99,122,.15)', info:'rgba(179,82,122,.15)' };
    const borders = { success:'rgba(217,192,143,.4)',  error:'rgba(224,99,122,.4)', info:'rgba(179,82,122,.4)'  };

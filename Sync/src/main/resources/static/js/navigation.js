...
const Navigation = (() => {
  ...
  function _doSwitch(pageId, context) {
    ...
    // 홈 화면을 벗어나면 갤럭시 뷰(모드 토글 포함)는 숨김 — React에서 Home이 언마운트되며 사라지는 것과 동일
    const modeToggle = document.getElementById('home-mode-toggle');
    if (modeToggle) modeToggle.style.display = pageId === 'home' ? '' : 'none';
    if (pageId !== 'home' && typeof GalaxyView !== 'undefined') GalaxyView.hide();
...
const ProfilePage = (() => {
  ...
  function populate() {
    ...
    if (nameEl)   nameEl.textContent   = user.display_name || user.username || 'SYNC 사용자';
    ...
    if (!section) return;
      ...
      section.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;
                    padding:16px;background:${expired ? 'rgba(255,165,0,.08)' : 'rgba(217,192,143,.08)'};
                    border:1px solid ${expired ? 'rgba(255,165,0,.3)' : 'rgba(217,192,143,.2)'};
                    border-radius:12px">
          <div>
            <div style="font-size:14px;font-weight:700;color:${expired ? '#ffa500' : 'var(--accent)'}">
              ${expired ? '⚠ 토큰 만료' : '✅ 연동됨'}
            </div>
      ...
      section.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;
          ...
          <button onclick="ProfilePage._connectSpotify()"
                  style="background:var(--accent);border:none;color:#000;padding:8px 18px;
                         border-radius:50px;font-size:12px;font-weight:700;
...
const SyncPage = (() => {
  ...
  function init() {
    if (!initialized) { _renderMoods(); initialized = true; }
    SyncPage.renderIdle(); // 방문할 때마다 최근 감정 기록을 새로 불러옴 (sync-api.js가 실제 구현으로 덮어씀)
  }
  ...
  function _renderMoods() {
    ...
    c.innerHTML = SYNC_MOODS.map(m =>
      `<button class="mood-chip" onclick="this.classList.toggle('active')">${m}</button>`
    ).join('');
  }

  // sendMessage/removeImage/playPreview/feedback/renderIdle 은 js/sync-api.js 가 실제 백엔드 연동 버전으로 덮어씀
  function sendMessage() {}
  function removeImage() {}
  function renderIdle() {}

  return { init, sendMessage, removeImage, renderIdle };
})();

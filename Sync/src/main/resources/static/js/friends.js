...
const FriendRequestsModal = (() => {
  ...
  async function updateBadge() {
    ...
    const bannerCount = document.getElementById('req-banner-count');
    if (banner) banner.classList.toggle('hidden', count === 0);
    if (bannerCount) bannerCount.textContent = `${count}명이 친구 요청을 보냈어요`;
  }

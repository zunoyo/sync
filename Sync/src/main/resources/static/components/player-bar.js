/**
 * SYNC — Player Bar Component (하단 재생바)
 * id="player-root" 요소를 플레이어 HTML로 교체합니다.
 */
(function () {
...
<div class="player-wrapper">
  <div class="player-bar">
    ...
    <div class="player-track">
      <div id="player-art" class="player-art grad-4">🎤</div>
      <div class="player-track-info">
        <div id="player-name"   class="player-track-name">Supernova</div>
        <div id="player-artist" class="player-track-artist">aespa</div>
      </div>
      <div class="player-track-btns">
        <button id="player-like-btn" class="icon-btn liked" style="color:var(--accent)"
    ...
    <div class="player-center">
      <div class="player-controls">
        <button id="shuffle-btn" class="ctrl-btn" onclick="toggleShuffle()" title="랜덤 재생">
        ...
        <button class="ctrl-btn" onclick="prevTrack()" title="이전 곡">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6 8.5 6V6z"/></svg>
        </button>
        <button id="play-btn" class="play-ctrl-btn" onclick="togglePlay()" title="재생/일시정지">
          <svg class="pause-icon" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>
          <svg class="play-icon"  width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
        </button>
        <button class="ctrl-btn" onclick="nextTrackBtn()" title="다음 곡">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg>
        </button>
        <button id="repeat-btn" class="ctrl-btn" onclick="toggleRepeat()" title="반복 재생">
      ...
      <div class="progress-area">
        <span class="progress-time" id="current-time">1:07</span>
        <div class="progress-bar" id="progress-bar">
          <div class="progress-fill" id="progress-fill" style="width:35%"></div>
        </div>
        <span class="progress-time end" id="end-time">3:13</span>
      </div>
    </div>

    <!-- 오른쪽: 볼륨 + 패널 토글 -->
    <div class="player-right">
      <button id="lyrics-toggle-btn" class="ctrl-btn" title="가사 자동스크롤"
              onclick="RightPanel.toggleLyricsScroll()">
      ...
      <button class="ctrl-btn" title="다음 곡 목록" onclick="RightPanel.toggleQueueView()">
      ...
      <div class="volume-area">
        <button class="ctrl-btn" title="볼륨">
      ...
      <button id="panel-toggle-btn" class="panel-toggle-btn active"
              onclick="togglePanel()" title="Now Playing 패널">
